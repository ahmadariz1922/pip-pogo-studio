export default {
  async fetch(request, env) {
    ...
    if (request.method !== "POST") return new Response("Pip & Pogo API is ready!", { headers });
    ...
    try {
      ...
      if (action === "image") {
        ...
        if (!scene) return json({ error: { message: "Scene text is required." } }, 400, headers);
...
Style: bright polished 3D children's animation, warm soft lighting, expressive faces, colorful detailed environment, ages 3-10. No text, no captions, no logos, no watermark.`;
        ...
        if (!response.ok) return json(data, response.status, headers);

        const b64 = data?.data?.[0]?.b64_json;
        if (!b64) return json({ error: { message: "No image data was returned." } }, 500, headers);

        return json({ image: `data:image/png;base64,${b64}` }, 200, headers);
      }

      const prompt = String(body.prompt || "").trim();
      if (!prompt) return json({ error: { message: "Story prompt is required." } }, 400, headers);
      ...
      return json({ error: { message: error.message || "Unexpected server error." } }, 500, headers);
    }
  }
};
